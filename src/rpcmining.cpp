...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "global is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "global is downloading blocks...");

    if (pindexBest->nHeight >= LAST_POW_BLOCK && pindexBest->nHeight < NEW_POW_BLOCK)
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "global is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "global is downloading blocks...");
